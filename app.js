...
function temaUygula(mod) {
  const iosMu = mod === "ios" || mod === "sistem";
...
function PlakaGirisi({ il, harf, rakam, onIl, onHarf, onRakam }) {
  ...
  return /* @__PURE__ */ React.createElement(
    ...
    /* @__PURE__ */ React.createElement("input", { style: { ...S.inp, width: 56, textAlign: "center" }, inputMode: "numeric", pattern: "[0-9]*", placeholder: "34", maxLength: 2, value: il || "", onChange: (e) => {
      const deger = e.target.value.replace(/[^0-9]/g, "").slice(0, 2);
      onIl(deger);
      if (deger.length === 2 && harfRef.current) harfRef.current.focus();
    } }),
    /* @__PURE__ */ React.createElement("input", { ref: harfRef, style: { ...S.inp, flex: 1, textAlign: "center" }, autoCapitalize: "characters", placeholder: "ABC", maxLength: 3, value: harf || "", onChange: (e) => {
      const deger = e.target.value.toUpperCase().replace(/[^A-Z]/g, "").slice(0, 3);
      onHarf(deger);
      if (deger.length === 3 && rakamRef.current) rakamRef.current.focus();
    } }),
    /* @__PURE__ */ React.createElement("input", { ref: rakamRef, style: { ...S.inp, width: 100, textAlign: "center" }, inputMode: "numeric", pattern: "[0-9]*", placeholder: "12345", maxLength: 5, value: rakam || "", onChange: (e) => onRakam(e.target.value.replace(/[^0-9]/g, "").slice(0, 5)) })
  );
}
const ARAC_GRUP_LABEL = { otomobil: "\u{1F697} Otomobil / Ticari", motosiklet: "\u{1F3CD}\uFE0F Motosiklet", traktor: "\u{1F69C} Traktör", kamyon: "\u{1F69B} Kamyon / Ağır Vasıta", is_makinesi: "\u{1F3D7}\uFE0F İş Makinesi", diger: "\u{1F4E6} Diğer" };
function MarkaModelSecici({ grup, marka, model, onGrup, onMarka, onModel }) {
  const markaListId = useId();
  const modelListId = useId();
  const g = grup || "otomobil";
  const markalar = g === "motosiklet" ? ARAC_MARKALARI_MOTOSIKLET : g === "traktor" ? ARAC_MARKALARI_TRAKTOR : g === "kamyon" ? ARAC_MARKALARI_KAMYON : g === "is_makinesi" ? ARAC_MARKALARI_IS_MAKINESI : g === "diger" ? [] : ARAC_MARKALARI;
  const modelHaritasi = g === "motosiklet" ? ARAC_MODELLERI_MOTOSIKLET : g === "traktor" ? ARAC_MODELLERI_TRAKTOR : g === "kamyon" ? ARAC_MODELLERI_KAMYON : g === "is_makinesi" ? ARAC_MODELLERI_IS_MAKINESI : g === "diger" ? {} : ARAC_MODELLERI;
...
const ARAC_MODELLERI = {
  "Renault": ["Clio", "Clio HB", "Clio SW", "Megane", "Megane HB", "Megane Sedan", "Megane E-Tech", "Symbol", "Fluence", "Talisman", "Kadjar", "Captur", "Austral", "Arkana", "Taliant", "Broadway", "Kangoo", "Kangoo Van", "Master", "Trafic", "Twingo", "Scenic", "Grand Scenic", "Laguna", "Espace", "Zoe", "9", "12", "R11", "R19", "Toros"],
  "Fiat": ["Egea", "Egea Cross", "Egea Sedan", "Egea HB", "Egea SW", "Linea", "Punto", "Grande Punto", "Doblo", "Doblo Combi", "Fiorino", "Tipo", "Tipo HB", "Tipo SW", "500", "500X", "500L", "Panda", "Ducato", "Talento", "Scudo", "Albea", "Palio", "Uno", "Marea", "Bravo", "Stilo", "Croma"],
  "Ford": ["Focus", "Focus HB", "Focus SW", "Fiesta", "Mondeo", "Mondeo SW", "Kuga", "Puma", "Courier", "Tourneo Courier", "Connect", "Tourneo Connect", "Transit", "Transit Custom", "Transit Courier", "Ranger", "EcoSport", "Galaxy", "S-Max", "B-Max", "C-Max", "Grand C-Max", "Explorer", "Edge", "Escort", "Taunus"],
  "Volkswagen": ["Passat", "Passat Variant", "Golf", "Golf Variant", "Polo", "Polo Sedan", "Jetta", "Tiguan", "Tiguan Allspace", "Caddy", "Caddy Van", "Transporter", "Crafter", "Bora", "Scirocco", "T-Roc", "T-Cross", "Taigo", "Arteon", "Amarok", "ID.3", "ID.4", "Beetle", "Vento", "Multivan"],
  "Toyota": ["Corolla", "Corolla Sedan", "Corolla HB", "Corolla Cross", "Yaris", "Yaris Cross", "Auris", "C-HR", "RAV4", "Hilux", "Avensis", "Camry", "Land Cruiser", "Land Cruiser Prado", "Proace", "Proace City", "Aygo", "Verso", "Prius", "Highlander", "Corona"],
  "Hyundai": ["i20", "i10", "i30", "i30 N", "Accent", "Accent Blue", "Accent Era", "Elantra", "Tucson", "Bayon", "Kona", "Santa Fe", "ix35", "ix20", "Custin", "Staria", "Getz", "Matrix", "H-1", "H100", "Sonata", "Venue"],
  "Peugeot": ["301", "308", "308 SW", "208", "3008", "2008", "508", "508 SW", "Partner", "Boxer", "5008", "Expert", "406", "407", "206", "207", "301 Sedan", "Rifter", "Traveller", "Bipper"],
  "Citroën": ["C-Elysee", "C3", "C3 Aircross", "C4", "C4 Cactus", "C4 X", "Berlingo", "Jumpy", "Jumper", "C5", "C5 Aircross", "C5 X", "Xsara", "Xsara Picasso", "C2", "Saxo", "Nemo"],
  "Opel": ["Astra", "Astra HB", "Astra Sedan", "Corsa", "Insignia", "Insignia SportsTourer", "Mokka", "Combo", "Vectra", "Vivaro", "Grandland", "Crossland", "Zafira", "Meriva", "Karl"],
  "Mercedes-Benz": ["A Serisi", "B Serisi", "C Serisi", "C Serisi Sedan", "C Serisi Estate", "E Serisi", "E Serisi Sedan", "E Serisi Estate", "S Serisi", "CLA", "CLA Shooting Brake", "GLA", "GLB", "GLC", "GLE", "GLS", "Vito", "Vito Tourer", "Sprinter", "Actros", "Axor", "Atego", "V Serisi", "CLS", "SLK", "EQA", "EQB", "180", "190"],
  "BMW": ["1 Serisi", "2 Serisi", "2 Serisi Gran Coupe", "3 Serisi", "3 Serisi Touring", "4 Serisi", "5 Serisi", "5 Serisi Touring", "6 Serisi", "7 Serisi", "8 Serisi", "X1", "X2", "X3", "X4", "X5", "X6", "X7", "iX", "i4", "Z4"],
  "Audi": ["A1", "A3", "A3 Sedan", "A4", "A4 Avant", "A5", "A6", "A6 Avant", "A7", "A8", "Q2", "Q3", "Q4 e-tron", "Q5", "Q7", "Q8", "TT", "e-tron GT"],
  "Skoda": ["Octavia", "Octavia Combi", "Fabia", "Fabia Combi", "Superb", "Superb Combi", "Rapid", "Rapid Spaceback", "Karoq", "Kodiaq", "Scala", "Kamiq", "Yeti", "Roomster", "Enyaq"],
  "Dacia": ["Duster", "Sandero", "Sandero Stepway", "Logan", "Logan MCV", "Lodgy", "Dokker", "Dokker Van", "Jogger", "Spring", "Solenza"],
  "Nissan": ["Micra", "Qashqai", "Juke", "Note", "X-Trail", "Navara", "Primera", "NV200", "Almera", "Pathfinder", "Pulsar", "Terrano", "Sunny", "Patrol"],
  "Honda": ["Civic", "Civic Sedan", "Civic HB", "City", "CR-V", "Jazz", "HR-V", "Accord", "e:Ny1", "FR-V", "Insight"],
  "Kia": ["Rio", "Ceed", "Ceed SW", "ProCeed", "Sportage", "Picanto", "Stonic", "Sorento", "Niro", "Sportage Plug-in", "Carnival", "Cerato", "Soul", "XCeed", "EV6"],
  "Chevrolet": ["Cruze", "Cruze HB", "Aveo", "Lacetti", "Captiva", "Spark", "Malibu", "Orlando", "Nubira", "Rezzo", "Epica"],
  "Seat": ["Ibiza", "Leon", "Leon SW", "Toledo", "Ateca", "Arona", "Tarraco", "Cordoba", "Alhambra"],
  "Volvo": ["S60", "S40", "S80", "S90", "V40", "V50", "V60", "V90", "XC40", "XC60", "XC90", "FH", "FM", "FMX"],
  "Mazda": ["2", "3", "6", "CX-3", "CX-30", "CX-5", "CX-60", "CX-9", "MX-5", "5"],
  "Mitsubishi": ["Lancer", "L200", "Outlander", "ASX", "Colt", "Space Star", "Eclipse Cross", "Pajero", "Carisma", "Grandis"],
  "Suzuki": ["Swift", "Vitara", "Baleno", "S-Cross", "Jimny", "Celerio", "Ignis", "Alto"],
  ...
  "Tofaş": ["Şahin", "Doğan", "Kartal", "Murat 131", "Murat 124", "Serçe", "Şahin Slx", "Doğan SLX", "Doğan Steyer"],
  ...
  "Lada": ["Niva", "Granta", "Vesta", "Kalina", "Priora", "2107", "Samara"],
...
const ARAC_MARKALARI_KAMYON = [
  "Mercedes-Benz", "MAN", "Scania", "DAF", "Iveco", "Volvo", "Renault Trucks", "Ford Trucks",
  "Isuzu", "Hyundai", "BMC", "Otokar", "Karsan", "Temsa", "Anadolu Isuzu", "Diğer"
];
const ARAC_MODELLERI_KAMYON = {
  "Mercedes-Benz": ["Actros", "Axor", "Atego", "Antos", "Arocs"],
  "MAN": ["TGX", "TGS", "TGL", "TGM", "TGE"],
  "Scania": ["R Serisi", "P Serisi", "G Serisi", "S Serisi"],
  "DAF": ["XF", "CF", "LF"],
  "Iveco": ["Daily", "Eurocargo", "Stralis", "S-Way", "Trakker"],
  "Volvo": ["FH", "FM", "FMX", "FL", "FE"],
  "Renault Trucks": ["T Serisi", "D Serisi", "K Serisi", "Master"],
  "Ford Trucks": ["Cargo", "F-Max", "Transit", "Trakker"],
  "Isuzu": ["D-Max", "NPR", "NQR", "N-Serisi"],
  "Hyundai": ["HD35", "HD65", "HD78", "Mighty"],
  "BMC": ["Fatih", "Pro", "Neco", "Tugra"],
  "Otokar": ["Atlas", "Kamyon", "Vectio"],
  "Karsan": ["Jest", "Atak", "Star", "Midi"],
  "Temsa": ["MD9", "HD12"],
  "Anadolu Isuzu": ["Kirazlıtepe", "Novociti", "Turkuaz"]
};
const ARAC_MARKALARI_IS_MAKINESI = [
  "Caterpillar", "Komatsu", "JCB", "Hitachi", "Volvo CE", "Liebherr", "Hyundai Construction",
  "Doosan", "Case CE", "New Holland CE", "Kobelco", "Bobcat", "Fiat-Hitachi", "Terex", "Diğer"
];
const ARAC_MODELLERI_IS_MAKINESI = {
  "Caterpillar": ["320", "330", "950", "966", "428", "D6"],
  "Komatsu": ["PC200", "PC300", "WA320", "D65"],
  "JCB": ["3CX", "4CX", "JS130", "JS220"],
  "Hitachi": ["ZX130", "ZX210", "ZX350"],
  "Volvo CE": ["EC210", "EC300", "L60", "L90"],
  "Liebherr": ["R 906", "R 924", "A 900"],
  "Hyundai Construction": ["R140", "R220", "HL730"],
  "Doosan": ["DX140", "DX225", "DL200"],
  "Case CE": ["CX130", "CX210", "580"],
  "New Holland CE": ["E135", "E215", "B95"],
  "Kobelco": ["SK140", "SK210"],
  "Bobcat": ["S650", "T650", "E35"],
  "Fiat-Hitachi": ["FH130", "FH200"],
  "Terex": ["TX140", "TX210"]
};
...
function bildirimleriKontrolEt() {
  ...
  const gunSonraTarih = (gun) => {
    const d = /* @__PURE__ */ new Date();
    d.setDate(d.getDate() + gun);
    return d.toISOString().slice(0, 10);
  };
  const isEmriHatirlatmaGunu = Math.max(0, +ayar.randevuHatirlatmaGunuIsEmri || 0) || 1;
  const isEmriHedefTarih = gunSonraTarih(isEmriHatirlatmaGunu);
  const yaklasanRandevular = servisler.filter((s) => s.tarih === isEmriHedefTarih && s.durum !== "iptal");
  if (yaklasanRandevular.length > 0) {
    bildirimGoster("\u{1F4C5} Yaklaşan Randevular", `${isEmriHatirlatmaGunu} g\xFCn sonra ${yaklasanRandevular.length} iş emri randevunuz var.`);
  }
  const elArabasiHatirlatmaGunu = Math.max(0, +ayar.randevuHatirlatmaGunuElArabasi || 0) || 1;
  const elArabasiHedefTarih = gunSonraTarih(elArabasiHatirlatmaGunu);
  const yaklasanSatislar = satislar.filter((s) => s.tarih === elArabasiHedefTarih);
  if (yaklasanSatislar.length > 0) {
    bildirimGoster("\u{1F6D2} Yaklaşan El Arabası Randevuları", `${elArabasiHatirlatmaGunu} g\xFCn sonra ${yaklasanSatislar.length} el arabası randevunuz var.`);
  }
  const tahsilatHatirlatmaGunu = Math.max(0, +ayar.tahsilatHatirlatmaGunu || 0);
...
const DEFAULT_SETTINGS = {
  ...
  randevuHatirlatmaGunuIsEmri: 1,
  randevuHatirlatmaGunuElArabasi: 1,
...
const S = {
  ...
  get sel() { return { width: "100%", background: C.surface, border: `1px solid ${C.border}`, borderRadius: 8 * RADIUS_OLCEK, padding: "9px 12px", color: C.white, fontSize: 13.5 }; },
  ...
  get mbox() { return { background: C.card, border: GOLGE_AKTIF ? "none" : `1px solid ${C.border}`, boxShadow: GOLGE_AKTIF ? "0 8px 30px rgba(0,0,0,0.15)" : "none", borderRadius: 14 * RADIUS_OLCEK, padding: 24, maxHeight: "88vh", overflowY: "auto", overflowX: "auto" }; },
...
function SiraliTablo({ columns, rows, rowKey, bosMesaj = "Kayıt yok.", className, dosyaAdi, baslik, topluIslem, iceAktar }) {
  ...
  return React.createElement(
    ...
    React.createElement(
      ...
      React.createElement(
        ...
        iceAktar && React.createElement(IceAktarButonu, { alanlar: iceAktar.alanlar, onIceAktar: iceAktar.onIceAktar })
      ),
...
function Takvim({ sayfayaGit, hedefeGit } = {}) {
  const [servisler] = useState(LS.get("servisIsleri"));
  const [cariler] = useState(LS.get("cariler"));
  const [araclar] = useState(LS.get("araclar"));
  const [satislar] = useState(LS.get("satislar"));
  const [gorevler] = useState(LS.get("yapilacaklar"));
  const su = /* @__PURE__ */ new Date();
  const [yil, setYil] = useState(su.getFullYear());
  const [ay, setAy] = useState(su.getMonth());
  const [seciliGun, setSeciliGun] = useState(today());
  const [topluHatirlatIndex, setTopluHatirlatIndex] = useState(null);
  const [tipSecimModal, setTipSecimModal] = useState(false);
  const [saatModal, setSaatModal] = useState(false);
  const [randevuSaat, setRandevuSaat] = useState("09:00");
  ...
  const gununSatislari = (tarih) => satislar.filter((s) => s.tarih === tarih);
  const gununGorevleri = (tarih) => gorevler.filter((g) => g.bitisTarihi === tarih && !g.tamamlandi);
  const bugunIsleri = gununIsleri(today());
  const seciliIsleri = gununIsleri(seciliGun);
  const seciliSatislar = gununSatislari(seciliGun);
  const seciliGorevler = gununGorevleri(seciliGun);
  ...
  const tipSecildi = (tip) => {
    setTipSecimModal(false);
    if (tip === "is_emri") {
      setRandevuSaat("09:00");
      setSaatModal(true);
    } else if (tip === "el_arabasi") {
      hedefeGit && hedefeGit("el_arabasi", { tip: "yeni_satis", tarih: seciliGun });
    } else {
      hedefeGit && hedefeGit("yapilacaklar", { tip: "yeni_gorev", tarih: seciliGun, tur: tip });
    }
  };
  const randevuOlustur = () => {
    setSaatModal(false);
    hedefeGit && hedefeGit("servis", { tip: "yeni_is_emri", tarih: seciliGun, saat: randevuSaat });
  };
  ...
  return /* @__PURE__ */ React.createElement(
    ...
    /* @__PURE__ */ React.createElement("div", { style: { display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16, flexWrap: "wrap", gap: 10 } },
      /* @__PURE__ */ React.createElement("div", { style: { fontSize: 20, fontWeight: 800, color: C.white } }, "📅 Takvim"),
    ...
    /* @__PURE__ */ React.createElement(
      ...
      /* @__PURE__ */ React.createElement("div", { style: { display: "grid", gridTemplateColumns: "repeat(7,1fr)", gap: 4 } }, gunler.map((g, i) => {
        ...
        const toplamSayi = gununIsleri(tarih).length + gununSatislari(tarih).length + gununGorevleri(tarih).length;
        ...
        return /* @__PURE__ */ React.createElement(
          ...
          toplamSayi > 0 && /* @__PURE__ */ React.createElement("div", { style: { marginTop: 2 } }, /* @__PURE__ */ React.createElement("span", { style: { ...S.badge(C.blue), fontSize: 9.5, padding: "1px 6px" } }, toplamSayi)),
    ...
    /* @__PURE__ */ React.createElement(
      ...
      /* @__PURE__ */ React.createElement("div", { style: { display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 8, marginBottom: 4 } },
        /* @__PURE__ */ React.createElement("div", { style: S.secTitle }, fmtDate(seciliGun), " Tarihli Kayıtlar", gununResmiHatirlaticilari(+seciliGun.slice(8, 10)).length > 0 && /* @__PURE__ */ React.createElement("span", { style: { ...S.badge(C.yellow), fontSize: 10.5, marginLeft: 8 } }, "\u{1F4C4} ", gununResmiHatirlaticilari(+seciliGun.slice(8, 10)).map((r) => r.ad).join(", "))),
        React.createElement("div", { style: { display: "flex", gap: 8, flexWrap: "wrap" } },
          hedefeGit && React.createElement("button", { style: { ...S.btnO, padding: "5px 10px", fontSize: 11 }, onClick: () => setTipSecimModal(true) }, "➕ Bu Tarihe Ekle"),
      ...
      (seciliIsleri.length + seciliSatislar.length + seciliGorevler.length) === 0
        ? /* @__PURE__ */ React.createElement("div", { style: { color: C.muted, fontSize: 13 } }, "Bu tarihte kayıtlı bir şey yok.")
        : /* @__PURE__ */ React.createElement("div", { style: { display: "flex", flexDirection: "column", gap: 8 } },
            seciliIsleri.map((s) => /* @__PURE__ */ React.createElement(
              "div",
              { key: "servis-" + s.id, style: { display: "flex", justifyContent: "space-between", alignItems: "center", padding: "10px 14px", background: C.surface, borderRadius: 8, flexWrap: "wrap", gap: 6 } },
              /* @__PURE__ */ React.createElement("span", { style: { fontSize: 13, color: C.text } }, /* @__PURE__ */ React.createElement("span", { style: S.badge(C.blue) }, "\u{1F527} İş Emri"), " ", /* @__PURE__ */ React.createElement("strong", { style: { color: C.white, marginLeft: 6 } }, s.isEmriNo), " — ", aracLabel(s), " — ", cariAd(cariler, s.musteriId), " — ", HIZMET_TIP_LABEL[s.hizmetTuru]),
            ...
            seciliSatislar.map((s) => /* @__PURE__ */ React.createElement(
              "div",
              { key: "satis-" + s.id, style: { display: "flex", justifyContent: "space-between", alignItems: "center", padding: "10px 14px", background: C.surface, borderRadius: 8, flexWrap: "wrap", gap: 6 } },
              /* @__PURE__ */ React.createElement("span", { style: { fontSize: 13, color: C.text } }, /* @__PURE__ */ React.createElement("span", { style: S.badge(C.green) }, "\u{1F6D2} El Arabası"), " ", cariAd(cariler, s.musteriId), " — ", EL_ARABASI_TUR_LABEL[s.tur] || s.tur, " — ", fmtTL(s.toplam)),
              /* @__PURE__ */ React.createElement("span", { style: { fontSize: 11, color: C.muted } })
            )),
            seciliGorevler.map((g) => /* @__PURE__ */ React.createElement(
              "div",
              { key: "gorev-" + g.id, style: { display: "flex", justifyContent: "space-between", alignItems: "center", padding: "10px 14px", background: C.surface, borderRadius: 8, flexWrap: "wrap", gap: 6 } },
              /* @__PURE__ */ React.createElement("span", { style: { fontSize: 13, color: C.text } }, /* @__PURE__ */ React.createElement("span", { style: S.badge(g.tur === "hatirlatma" ? C.yellow : C.purple) }, g.tur === "hatirlatma" ? "\u{1F514} Hatırlatma" : "\u{1F4CB} Görev"), " ", g.baslik),
              /* @__PURE__ */ React.createElement("span", { style: { fontSize: 11, color: C.muted } })
            ))
          )
    ),
    tipSecimModal && React.createElement(
      Modal,
      { title: `➕ ${fmtDate(seciliGun)} Tarihine Ekle`, onClose: () => setTipSecimModal(false), width: 380 },
      React.createElement("div", { style: { display: "flex", flexDirection: "column", gap: 8 } },
        React.createElement("button", { type: "button", style: S.btnO, onClick: () => tipSecildi("is_emri") }, "\u{1F527} İş Emri Randevusu"),
        React.createElement("button", { type: "button", style: S.btnO, onClick: () => tipSecildi("el_arabasi") }, "\u{1F6D2} El Arabası Satış Randevusu"),
        React.createElement("button", { type: "button", style: S.btnO, onClick: () => tipSecildi("gorev") }, "\u{1F4CB} Görev"),
        React.createElement("button", { type: "button", style: S.btnO, onClick: () => tipSecildi("hatirlatma") }, "\u{1F514} Hatırlatma")
      )
    ),
    saatModal && React.createElement(
      Modal,
      { title: `➕ ${fmtDate(seciliGun)} Tarihine İş Emri Randevusu Ekle`, onClose: () => setSaatModal(false), width: 360 },
      React.createElement(FG, { label: "Saat" }, React.createElement("input", { type: "time", style: S.inp, value: randevuSaat, onChange: (e) => setRandevuSaat(e.target.value) })),
      React.createElement("div", { style: { fontSize: 11.5, color: C.muted, marginBottom: 14 } }, "Devam edince İş Emri sayfası bu tarih ve saatle önceden doldurulmuş şekilde açılır; müşteri, araç ve hizmet bilgilerini orada tamamlarsın."),
      React.createElement("div", { style: { display: "flex", gap: 10, justifyContent: "flex-end" } },
        React.createElement("button", { style: S.btnO, onClick: () => setSaatModal(false) }, "İptal"),
        React.createElement("button", { style: S.btn(), onClick: randevuOlustur }, "Devam Et")
      )
    )
  );
}
function ServisIsleri({ hedef, hedefTemizle, sayfayaGit } = {}) {
  ...
  const [analizHizmet, setAnalizHizmet] = useState("");
  const [analizPersonel, setAnalizPersonel] = useState("");
  const [analizBaslangic, setAnalizBaslangic] = useState("");
  const [analizBitis, setAnalizBitis] = useState("");
...
Tutar hakkında yeterli bilgi yoksa tutarOnerisi'ni null yap.`;
  ...
  const SI_RENKLER = [C.accent, C.blue, C.green, C.yellow, C.red, C.steel, C.purple, C.muted];
  const analizFiltreli = liste.filter((s) => s.durum !== "iptal" && (!analizHizmet || s.hizmetTuru === analizHizmet) && (!analizPersonel || s.personelId === analizPersonel) && (!analizBaslangic || s.tarih >= analizBaslangic) && (!analizBitis || s.tarih <= analizBitis));
  const analizToplamCiro = analizFiltreli.reduce((t, s) => t + (+s.tutar || 0), 0);
  const analizOrtIslem = analizFiltreli.length > 0 ? analizToplamCiro / analizFiltreli.length : 0;
  const analizTamamlanan = analizFiltreli.filter((s) => s.durum === "tamamlandi").length;
  const siHizmetDagilimi = Object.entries(analizFiltreli.reduce((acc, s) => {
    const h = HIZMET_TIP_LABEL[s.hizmetTuru] || s.hizmetTuru || "Diğer";
    acc[h] = (acc[h] || 0) + (+s.tutar || 0);
    return acc;
  }, {})).map(([name, value]) => ({ name, value })).sort((a, b) => b.value - a.value);
  const siPersonelDagilimi = Object.entries(analizFiltreli.reduce((acc, s) => {
    const p = personelListesi.find((x) => x.id === s.personelId);
    const ad = p ? p.ad : "Atanmamış";
    acc[ad] = (acc[ad] || 0) + (+s.tutar || 0);
    return acc;
  }, {})).map(([name, value]) => ({ name, value })).sort((a, b) => b.value - a.value);
  const siAylikTrend = (() => {
    const gruplar = {};
    analizFiltreli.forEach((s) => {
      const ay = (s.tarih || "").slice(0, 7);
      if (!ay) return;
      gruplar[ay] = (gruplar[ay] || 0) + (+s.tutar || 0);
    });
    return Object.entries(gruplar).sort((a, b) => a[0].localeCompare(b[0])).slice(-12).map(([ay, tutar]) => {
      const [y, m] = ay.split("-");
      return { ay: `${AY_ADLARI[+m - 1]?.slice(0, 3) || m} '${y.slice(2)}`, Ciro: tutar };
    });
  })();
  ...
  return React.createElement(
    ...
    React.createElement(
      ...
      React.createElement("div", { style: { display: "flex", gap: 8 } },
        React.createElement("button", { style: S.btnO, onClick: () => setAnalizAcik((v) => !v) }, "\u{1F4C8} Detay"),
        sayfayaGit && React.createElement("button", { style: S.btnO, onClick: () => sayfayaGit("takvim") }, "\u{1F4C5} Takvim"),
    ...
    React.createElement(Grid4, null,
      ...
      React.createElement(StatCard, { color: C.blue, icon: "\u{1F4C5}", value: liste.filter((s) => s.tarih === today() && s.durum !== "iptal").length, label: "Bugünkü İşler" })
    ),
    analizAcik && React.createElement(
      "div",
      { style: { ...S.card, marginBottom: 16 } },
      React.createElement("div", { style: S.secTitle }, "\u{1F4C8} Detaylı Analiz"),
      React.createElement("div", { style: { display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 16 } },
        React.createElement("select", { style: { ...S.sel, width: "auto" }, value: analizHizmet, onChange: (e) => setAnalizHizmet(e.target.value) },
          React.createElement("option", { value: "" }, "Tüm Hizmet Türleri"),
          Object.entries(HIZMET_TIP_LABEL).map(([k, l]) => React.createElement("option", { key: k, value: k }, l))
        ),
        React.createElement("select", { style: { ...S.sel, width: "auto" }, value: analizPersonel, onChange: (e) => setAnalizPersonel(e.target.value) },
          React.createElement("option", { value: "" }, "Tüm Personel"),
          personelListesi.map((p) => React.createElement("option", { key: p.id, value: p.id }, p.ad))
        ),
        React.createElement("input", { type: "date", style: { ...S.inp, width: "auto" }, value: analizBaslangic, onChange: (e) => setAnalizBaslangic(e.target.value) }),
        React.createElement("input", { type: "date", style: { ...S.inp, width: "auto" }, value: analizBitis, onChange: (e) => setAnalizBitis(e.target.value) })
      ),
      React.createElement(Grid4, null,
        React.createElement(StatCard, { color: C.accent, icon: "\u{1F527}", value: analizFiltreli.length, label: "Filtrelenen İş" }),
        React.createElement(StatCard, { color: C.blue, icon: "\u{1F4B0}", value: fmtTL(analizToplamCiro), label: "Toplam Ciro" }),
        React.createElement(StatCard, { color: C.green, icon: "\u{1F4CA}", value: fmtTL(analizOrtIslem), label: "Ortalama İşlem" }),
        React.createElement(StatCard, { color: C.yellow, icon: "✅", value: analizTamamlanan, label: "Tamamlanan" })
      ),
      React.createElement(Grid2, null,
        React.createElement("div", { style: { ...S.card, marginTop: 14 } },
          React.createElement("div", { style: S.secTitle }, "\u{1F527} Hizmet Türüne Göre Ciro"),
          RC.PieChart && siHizmetDagilimi.length > 0 ? React.createElement(ResponsiveContainer, { width: "100%", height: 220 }, React.createElement(PieChart, null, React.createElement(Pie, { data: siHizmetDagilimi, dataKey: "value", nameKey: "name", cx: "50%", cy: "50%", innerRadius: 45, outerRadius: 78, paddingAngle: 3, label: ({ name, value }) => `${name}: ${fmtTL(value)}` }, siHizmetDagilimi.map((e, i) => React.createElement(Cell, { key: i, fill: SI_RENKLER[i % SI_RENKLER.length] }))), React.createElement(Tooltip, { contentStyle: { background: C.card, border: `1px solid ${C.border}`, borderRadius: 8 }, formatter: (v) => fmtTL(v) }))) : React.createElement("div", { style: { color: C.muted, fontSize: 13 } }, "Veri yok.")
        ),
        React.createElement("div", { style: { ...S.card, marginTop: 14 } },
          React.createElement("div", { style: S.secTitle }, "\u{1F9D1}‍\u{1F527} Personele Göre Ciro"),
          RC.PieChart && siPersonelDagilimi.length > 0 ? React.createElement(ResponsiveContainer, { width: "100%", height: 220 }, React.createElement(PieChart, null, React.createElement(Pie, { data: siPersonelDagilimi, dataKey: "value", nameKey: "name", cx: "50%", cy: "50%", innerRadius: 45, outerRadius: 78, paddingAngle: 3, label: ({ name, value }) => `${name}: ${fmtTL(value)}` }, siPersonelDagilimi.map((e, i) => React.createElement(Cell, { key: i, fill: SI_RENKLER[i % SI_RENKLER.length] }))), React.createElement(Tooltip, { contentStyle: { background: C.card, border: `1px solid ${C.border}`, borderRadius: 8 }, formatter: (v) => fmtTL(v) }))) : React.createElement("div", { style: { color: C.muted, fontSize: 13 } }, "Veri yok.")
        )
      ),
      React.createElement("div", { style: { ...S.card, marginTop: 14 } },
        React.createElement("div", { style: S.secTitle }, "\u{1F4C8} Aylık Ciro Trendi"),
        RC.BarChart && siAylikTrend.length > 0 ? React.createElement(ResponsiveContainer, { width: "100%", height: 220 }, React.createElement(BarChart, { data: siAylikTrend },
          React.createElement(CartesianGrid, { strokeDasharray: "3 3", stroke: C.border }),
          React.createElement(XAxis, { dataKey: "ay", tick: { fill: C.muted, fontSize: 10 } }),
          ...
          React.createElement(Bar, { dataKey: "Ciro", fill: C.accent, radius: [4, 4, 0, 0] })
        )) : React.createElement("div", { style: { color: C.muted, fontSize: 13 } }, "Veri yok.")
      )
    ),
    React.createElement(
      "div",
...
function ElArabasi({ hedef, hedefTemizle } = {}) {
  ...
  const [detayTur, setDetayTur] = useState("");
  const [detayBolge, setDetayBolge] = useState("");
  const [detayBaslangic, setDetayBaslangic] = useState("");
  const [detayBitis, setDetayBitis] = useState("");
  const [takvimAcik, setTakvimAcik] = useState(false);
  const eaSuAn = /* @__PURE__ */ new Date();
  const [takvimYil, setTakvimYil] = useState(eaSuAn.getFullYear());
  const [takvimAy, setTakvimAy] = useState(eaSuAn.getMonth());
  const [takvimSeciliGun, setTakvimSeciliGun] = useState(today());
  useEffect(() => {
    if (!hedef) return;
  ...
  const bolgeCikar = (musteriId) => {
    ...
    const parcalar = c.adres.split(",");
    return parcalar[parcalar.length - 1].trim() || "Belirtilmemiş";
  };
  const tumBolgeler = [...new Set(satislar.map((s) => bolgeCikar(s.musteriId)))].sort();
  const detayFiltreli = satislar.filter((s) => (!detayTur || s.tur === detayTur) && (!detayBolge || bolgeCikar(s.musteriId) === detayBolge) && (!detayBaslangic || s.tarih >= detayBaslangic) && (!detayBitis || s.tarih <= detayBitis));
  const detayToplamCiro = detayFiltreli.reduce((t, s) => t + (+s.toplam || 0), 0);
  const detayOrtalamaSepet = detayFiltreli.length > 0 ? detayToplamCiro / detayFiltreli.length : 0;
  const bolgeDagilimi = Object.entries(detayFiltreli.reduce((acc, s) => {
    const b = bolgeCikar(s.musteriId);
    acc[b] = (acc[b] || 0) + (+s.toplam || 0);
    return acc;
  }, {})).map(([name, value]) => ({ name, value })).sort((a, b) => b.value - a.value).slice(0, 8);
  const turDagilimi = Object.entries(detayFiltreli.reduce((acc, s) => {
    const t = EL_ARABASI_TUR_LABEL[s.tur] || s.tur || "Diğer";
    acc[t] = (acc[t] || 0) + (+s.toplam || 0);
    return acc;
  }, {})).map(([name, value]) => ({ name, value }));
  const aylikTrend = (() => {
    const gruplar = {};
    detayFiltreli.forEach((s) => {
      const ay = (s.tarih || "").slice(0, 7);
      if (!ay) return;
      gruplar[ay] = (gruplar[ay] || 0) + (+s.toplam || 0);
    });
    return Object.entries(gruplar).sort((a, b) => a[0].localeCompare(b[0])).slice(-12).map(([ay, tutar]) => {
      const [y, m] = ay.split("-");
      return { ay: `${AY_ADLARI[+m - 1]?.slice(0, 3) || m} '${y.slice(2)}`, Ciro: tutar };
  ...
  const DETAY_RENKLER = [C.accent, C.blue, C.green, C.yellow, C.red, C.steel, C.purple, C.muted];
  const eaGunStr = (y, a, g) => `${y}-${String(a + 1).padStart(2, "0")}-${String(g).padStart(2, "0")}`;
  const eaIlkGun = new Date(takvimYil, takvimAy, 1);
  const eaGunSayisi = new Date(takvimYil, takvimAy + 1, 0).getDate();
  const eaBosluk = (eaIlkGun.getDay() + 6) % 7;
  const eaGunler = [];
  for (let i = 0; i < eaBosluk; i++) eaGunler.push(null);
  for (let g = 1; g <= eaGunSayisi; g++) eaGunler.push(g);
  const gununSatislari = (tarih) => satislar.filter((s) => s.tarih === tarih);
  const takvimSeciliSatislar = gununSatislari(takvimSeciliGun);
  const eaOncekiAy = () => {
    if (takvimAy === 0) { setTakvimAy(11); setTakvimYil((y) => y - 1); } else setTakvimAy((a) => a - 1);
  };
  const eaSonrakiAy = () => {
    if (takvimAy === 11) { setTakvimAy(0); setTakvimYil((y) => y + 1); } else setTakvimAy((a) => a + 1);
  };
  const eaRandevuEkle = () => {
    setForm({ tarih: takvimSeciliGun, kdvOrani: 0 });
    setHata("");
    setModalAcik(true);
  };
  ...
  return /* @__PURE__ */ React.createElement(
    "div",
    { className: "fp-fade" },
    /* @__PURE__ */ React.createElement("div", { style: { display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16, flexWrap: "wrap", gap: 10 } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 20, fontWeight: 800, color: C.white } }, "\u{1F6D2} El Arabası"), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 8 } },
      React.createElement("button", { style: S.btnO, onClick: () => setDetayAcik((v) => !v) }, "📈 Detay"),
      React.createElement("button", { style: S.btnO, onClick: () => setTakvimAcik((v) => !v) }, "\u{1F4C5} Takvim"),
      /* @__PURE__ */ React.createElement("button", { style: S.btn(), onClick: () => {
      setForm({ tarih: today(), kdvOrani: 0 });
      setHata("");
      setModalAcik(true);
    } }, "➕ Yeni Satış"))),
    ...
    detayAcik && React.createElement(
      "div",
      { style: { ...S.card, marginBottom: 16 } },
      React.createElement("div", { style: S.secTitle }, "📈 Detaylı Analiz"),
      React.createElement("div", { style: { display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 16 } },
        React.createElement("select", { style: { ...S.sel, width: "auto" }, value: detayTur, onChange: (e) => setDetayTur(e.target.value) },
          React.createElement("option", { value: "" }, "T\xFCm T\xFCrler"),
          Object.entries(EL_ARABASI_TUR_LABEL).map(([k, l]) => React.createElement("option", { key: k, value: k }, l))
        ),
        React.createElement("select", { style: { ...S.sel, width: "auto" }, value: detayBolge, onChange: (e) => setDetayBolge(e.target.value) },
          React.createElement("option", { value: "" }, "T\xFCm B\xF6lgeler"),
          tumBolgeler.map((b) => React.createElement("option", { key: b, value: b }, b))
        ),
        React.createElement("input", { type: "date", style: { ...S.inp, width: "auto" }, value: detayBaslangic, onChange: (e) => setDetayBaslangic(e.target.value) }),
        React.createElement("input", { type: "date", style: { ...S.inp, width: "auto" }, value: detayBitis, onChange: (e) => setDetayBitis(e.target.value) })
      ),
      React.createElement(Grid4, null,
        React.createElement(StatCard, { color: C.blue, icon: "\u{1F6D2}", value: detayFiltreli.length, label: "Filtrelenen Satış" }),
        React.createElement(StatCard, { color: C.accent, icon: "\u{1F4B0}", value: fmtTL(detayToplamCiro), label: "Filtrelenen Ciro" }),
        React.createElement(StatCard, { color: C.green, icon: "\u{1F4CA}", value: fmtTL(detayOrtalamaSepet), label: "Ortalama Sepet" }),
        React.createElement(StatCard, { color: C.yellow, icon: "\u{1F4CD}", value: tumBolgeler.length, label: "Farklı Bölge" })
      ),
      React.createElement(Grid2, null,
        React.createElement("div", { style: { ...S.card, marginTop: 14 } },
          React.createElement("div", { style: S.secTitle }, "\u{1F4CD} Bölgeye Göre Ciro"),
          RC.PieChart && bolgeDagilimi.length > 0 ? React.createElement(ResponsiveContainer, { width: "100%", height: 220 }, React.createElement(PieChart, null, React.createElement(Pie, { data: bolgeDagilimi, dataKey: "value", nameKey: "name", cx: "50%", cy: "50%", innerRadius: 45, outerRadius: 78, paddingAngle: 3, label: ({ name, value }) => `${name}: ${fmtTL(value)}` }, bolgeDagilimi.map((e, i) => React.createElement(Cell, { key: i, fill: DETAY_RENKLER[i % DETAY_RENKLER.length] }))), React.createElement(Tooltip, { contentStyle: { background: C.card, border: `1px solid ${C.border}`, borderRadius: 8 }, formatter: (v) => fmtTL(v) }))) : React.createElement("div", { style: { color: C.muted, fontSize: 13 } }, "Veri yok.")
        ),
        React.createElement("div", { style: { ...S.card, marginTop: 14 } },
          React.createElement("div", { style: S.secTitle }, "\u{1F6D2} T\xFCr\xFCne Göre Ciro"),
          RC.PieChart && turDagilimi.length > 0 ? React.createElement(ResponsiveContainer, { width: "100%", height: 220 }, React.createElement(PieChart, null, React.createElement(Pie, { data: turDagilimi, dataKey: "value", nameKey: "name", cx: "50%", cy: "50%", innerRadius: 45, outerRadius: 78, paddingAngle: 3, label: ({ name, value }) => `${name}: ${fmtTL(value)}` }, turDagilimi.map((e, i) => React.createElement(Cell, { key: i, fill: DETAY_RENKLER[i % DETAY_RENKLER.length] }))), React.createElement(Tooltip, { contentStyle: { background: C.card, border: `1px solid ${C.border}`, borderRadius: 8 }, formatter: (v) => fmtTL(v) }))) : React.createElement("div", { style: { color: C.muted, fontSize: 13 } }, "Veri yok.")
        )
      ),
      React.createElement("div", { style: { ...S.card, marginTop: 14 } },
        React.createElement("div", { style: S.secTitle }, "\u{1F4C8} Aylık Ciro Trendi"),
        RC.BarChart && aylikTrend.length > 0 ? React.createElement(ResponsiveContainer, { width: "100%", height: 220 }, React.createElement(BarChart, { data: aylikTrend },
          React.createElement(CartesianGrid, { strokeDasharray: "3 3", stroke: C.border }),
          React.createElement(XAxis, { dataKey: "ay", tick: { fill: C.muted, fontSize: 10 } }),
          React.createElement(YAxis, { tick: { fill: C.muted, fontSize: 10 } }),
          React.createElement(Tooltip, { contentStyle: { background: C.card, border: `1px solid ${C.border}`, borderRadius: 8 }, formatter: (v) => fmtTL(v) }),
    ...
    takvimAcik && React.createElement(
      "div",
      { style: { ...S.card, marginBottom: 16 } },
      React.createElement("div", { style: { display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 } }, React.createElement("button", { style: S.btnO, onClick: eaOncekiAy }, "◀"), React.createElement("div", { style: { fontWeight: 700, color: C.white } }, AY_ADLARI[takvimAy], " ", takvimYil), React.createElement("div", { style: { display: "flex", gap: 8 } }, React.createElement("button", { style: S.btnO, onClick: () => { setTakvimYil(eaSuAn.getFullYear()); setTakvimAy(eaSuAn.getMonth()); setTakvimSeciliGun(today()); } }, "Bugün"), React.createElement("button", { style: S.btnO, onClick: eaSonrakiAy }, "▶"))),
      React.createElement("div", { style: { display: "grid", gridTemplateColumns: "repeat(7,1fr)", gap: 4, marginBottom: 4 } }, GUN_ADLARI.map((g) => React.createElement("div", { key: g, style: { textAlign: "center", fontSize: 11, color: C.muted, fontWeight: 700 } }, g))),
      React.createElement("div", { style: { display: "grid", gridTemplateColumns: "repeat(7,1fr)", gap: 4, marginBottom: 16 } }, eaGunler.map((g, i) => {
        if (g === null) return React.createElement("div", { key: "b" + i });
        const tarih = eaGunStr(takvimYil, takvimAy, g);
        const gununSayisi = gununSatislari(tarih).length;
        const bugunMu = tarih === today();
        const seciliMi = tarih === takvimSeciliGun;
        return React.createElement(
          "div",
          { key: tarih, onClick: () => setTakvimSeciliGun(tarih), style: { padding: "8px 4px", borderRadius: 8, textAlign: "center", cursor: "pointer", minHeight: 44, background: seciliMi ? C.accent + "33" : bugunMu ? C.surface : "transparent", border: `1px solid ${seciliMi ? C.accent : bugunMu ? C.border : "transparent"}` } },
          React.createElement("div", { style: { fontSize: 12.5, color: bugunMu ? C.accent : C.text, fontWeight: bugunMu ? 800 : 400 } }, g),
          gununSayisi > 0 && React.createElement("div", { style: { marginTop: 2 } }, React.createElement("span", { style: { ...S.badge(C.blue), fontSize: 9.5, padding: "1px 6px" } }, gununSayisi))
        );
      })),
      React.createElement("div", { style: { display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 8, marginBottom: 10 } },
        React.createElement("div", { style: S.secTitle }, fmtDate(takvimSeciliGun), " Tarihli Satışlar (", takvimSeciliSatislar.length, ")"),
        React.createElement("button", { style: { ...S.btnO, padding: "5px 10px", fontSize: 11 }, onClick: eaRandevuEkle }, "➕ Bu Tarihe Randevu Ekle")
      ),
      takvimSeciliSatislar.length === 0 ? React.createElement("div", { style: { color: C.muted, fontSize: 13 } }, "Bu tarihte kayıtlı satış yok.") : React.createElement("div", { style: { display: "flex", flexDirection: "column", gap: 8 } }, takvimSeciliSatislar.map((s) => React.createElement(
        "div",
        ...
        React.createElement("span", { style: { fontSize: 13, color: C.text } }, React.createElement("strong", { style: { color: C.white } }, cariAd(cariler, s.musteriId)), " — ", EL_ARABASI_TUR_LABEL[s.tur] || s.tur, " — ", fmtTL(s.toplam))
      )))
    ),
...
function Yapilacaklar({ hedef, hedefTemizle, sayfayaGit } = {}) {
  ...
  useEffect(() => {
    if (!hedef) return;
    if (hedef.tip === "yeni_gorev") {
      setForm({ bitisTarihi: hedef.tarih || "", tur: hedef.tur === "hatirlatma" ? "hatirlatma" : "gorev" });
      setHata("");
      setModalAcik(true);
    }
    hedefTemizle && hedefTemizle();
  ...
  const kaydet = () => {
    ...
    const kayit = { ...form, id: form.id || uid(), oncelik: form.oncelik || "orta", tur: form.tur === "hatirlatma" ? "hatirlatma" : "gorev", olusturmaTarihi: form.olusturmaTarihi || today(), tamamlandi: !!form.tamamlandi };
    const yeni = form.id ? liste.map((x) => x.id === form.id ? kayit : x) : [...liste, kayit];
  ...
  return /* @__PURE__ */ React.createElement(
    ...
    /* @__PURE__ */ React.createElement("div", { style: { display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16, flexWrap: "wrap", gap: 10 } },
      ...
      /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 8, flexWrap: "wrap" } },
        sayfayaGit && /* @__PURE__ */ React.createElement("button", { style: S.btnO, onClick: () => sayfayaGit("takvim") }, "\u{1F4C5} Takvim"),
              ...
              /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 10, alignItems: "flex-start", flex: 1, minWidth: 200 } },
                ...
                /* @__PURE__ */ React.createElement("div", null,
                  ...
                  /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 8, flexWrap: "wrap", marginTop: 8 } },
                    g.tur === "hatirlatma" && /* @__PURE__ */ React.createElement("span", { style: S.badge(C.yellow) }, "\u{1F514} Hatırlatma"),
    ...
    modalAcik && /* @__PURE__ */ React.createElement(
      ...
      /* @__PURE__ */ React.createElement(FG, { label: "Açıklama (opsiyonel)" }, /* @__PURE__ */ React.createElement("textarea", { style: { ...S.inp, minHeight: 60 }, value: form.aciklama || "", onChange: (e) => setForm((f) => ({ ...f, aciklama: e.target.value })) })),
      /* @__PURE__ */ React.createElement(Grid2, null,
        /* @__PURE__ */ React.createElement(FG, { label: "T\xFCr" }, /* @__PURE__ */ React.createElement("select", { style: S.sel, value: form.tur === "hatirlatma" ? "hatirlatma" : "gorev", onChange: (e) => setForm((f) => ({ ...f, tur: e.target.value })) }, /* @__PURE__ */ React.createElement("option", { value: "gorev" }, "\u{1F4CB} G\xF6rev"), /* @__PURE__ */ React.createElement("option", { value: "hatirlatma" }, "\u{1F514} Hatırlatma"))),
...
function Cariler({ hedef, hedefTemizle } = {}) {
  ...
  return /* @__PURE__ */ React.createElement("div", { className: "fp-fade" }, /* @__PURE__ */ React.createElement("div", { style: { display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16, flexWrap: "wrap", gap: 10 } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 20, fontWeight: 800, color: C.white } }, "\u{1F465} Cariler"), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 8, flexWrap: "wrap" } }, /* @__PURE__ */ React.createElement("button", { style: S.btn(), onClick: () => {
    setForm({});
    setModalAcik(true);
    ...
    dosyaAdi: "cariler",
    iceAktar: { alanlar: [{ baslik: "Ad", key: "ad", zorunlu: true }, { baslik: "Telefon", key: "tel" }, { baslik: "Adres", key: "adres" }], onIceAktar: cariIceAktar },
    rowKey: (c) => c.id,
    bosMesaj: "Kayıt bulunamadı.",
    rows: filtreliListe,
    columns: [
...
Soru: ${dogalSoru.trim()}`;
  ...
  return React.createElement(
    ...
    sekme === "giderler" && React.createElement(
      "div",
      null,
      React.createElement("div", { style: { display: "flex", justifyContent: "flex-end", gap: 8, flexWrap: "wrap", marginBottom: 14 } }, React.createElement("button", { style: S.btn(), onClick: () => { setGiderForm({ tarih: today(), kategori: GIDER_KATEGORILERI[0] }); setHata(""); setFisHata(""); setGiderModal(true); } }, "\u2796 Yeni Gider Ekle")),
      React.createElement(
        "div",
        { style: S.card },
        React.createElement(SiraliTablo, {
          dosyaAdi: "giderler",
          iceAktar: { alanlar: [{ baslik: "Tarih", key: "tarih" }, { baslik: "Kategori", key: "kategori" }, { baslik: "A\xE7ıklama", key: "aciklama" }, { baslik: "Tutar", key: "tutar", zorunlu: true }], onIceAktar: giderIceAktar },
          rowKey: (g) => g.id,
...
function BildirimlerYoneticisi({ tahsilatGunu, onTahsilatGunuDegistir, randevuGunuIsEmri, randevuGunuElArabasi, onRandevuGunuDegistir, gununOzetiAktif, gununOzetiGunler, gununOzetiSaati, onGununOzetiDegistir } = {}) {
  ...
  return React.createElement(
    ...
    onRandevuGunuDegistir && React.createElement("div", { style: { marginTop: 16, paddingTop: 14, borderTop: `1px solid ${C.border}` } },
      React.createElement("strong", { style: { color: C.white, fontSize: 13, display: "block", marginBottom: 10 } }, "📅 Randevu Hatırlatıcıları"),
      React.createElement(Grid2, null,
        React.createElement(FG, { label: "İş Emri: Kaç Gün Kala Hatırlatılsın?" },
          React.createElement("input", { type: "number", min: 1, style: { ...S.inp, maxWidth: 140 }, value: randevuGunuIsEmri ?? 1, onChange: (e) => onRandevuGunuDegistir({ randevuHatirlatmaGunuIsEmri: +e.target.value }) })
        ),
        React.createElement(FG, { label: "El Arabası: Kaç Gün Kala Hatırlatılsın?" },
          React.createElement("input", { type: "number", min: 1, style: { ...S.inp, maxWidth: 140 }, value: randevuGunuElArabasi ?? 1, onChange: (e) => onRandevuGunuDegistir({ randevuHatirlatmaGunuElArabasi: +e.target.value }) })
        )
      ),
      React.createElement("div", { style: { fontSize: 11, color: C.muted } }, "Randevu tarihine bu kadar gün kala günlük hatırlatma bildirimi gösterilir.")
    ),
...
function Ayarlar() {
  ...
  sekme === "genel" && React.createElement(React.Fragment, null, /* @__PURE__ */ React.createElement(KatlanirKart, { title: "\u{1F3E2} Firma Bilgileri" }, /* @__PURE__ */ React.createElement(FG, { label: "Firma Ad\u0131" }, /* @__PURE__ */ React.createElement("input", { style: S.inp, value: form.firmaAdi || "", onChange: (e) => setForm((f) => ({ ...f, firmaAdi: e.target.value })) })), /* @__PURE__ */ React.createElement(Grid2, null, /* @__PURE__ */ React.createElement(FG, { label: "Telefon" }, /* @__PURE__ */ React.createElement("input", { style: S.inp, value: form.firmaTel || "", onChange: (e) => setForm((f) => ({ ...f, firmaTel: e.target.value })) })), /* @__PURE__ */ React.createElement(FG, { label: "Varsay\u0131lan KDV Oran\u0131" }, /* @__PURE__ */ React.createElement("select", { style: S.sel, value: form.kdvOrani ?? "", onChange: (e) => setForm((f) => ({ ...f, kdvOrani: +e.target.value })) }, (form.kdvOranlari || KDV_ORANLARI_VARSAYILAN).map((o) => /* @__PURE__ */ React.createElement("option", { key: o, value: o }, "%", o))))), /* @__PURE__ */ React.createElement(FG, { label: "Adres" }, /* @__PURE__ */ React.createElement("input", { style: S.inp, value: form.firmaAdres || "", onChange: (e) => setForm((f) => ({ ...f, firmaAdres: e.target.value })) }))), React.createElement(FirmaLogoYoneticisi, null), React.createElement(KatlanirKart, { title: "\u{1F3A8} G\xF6r\xFCn\xFCm" }, React.createElement("div", { style: { fontSize: 12, color: C.muted, marginBottom: 14 } }, "Uygulaman\u0131n renk temas\u0131n\u0131 se\xE7in."),
    ...
    React.createElement("button", { type: "button", style: S.btnO, onClick: resmiHatirlaticiEkle }, "\u2795 Yeni Hat\u0131rlat\u0131c\u0131 Ekle"))), sekme === "wp_sablon" && React.createElement(React.Fragment, null,
      ...
      Object.keys(MESAJ_SABLONU_LABEL).map((key) => React.createElement(
        KatlanirKart,
        { key, title: MESAJ_SABLONU_LABEL[key] },
...
function Araclar({ hedef, hedefTemizle } = {}) {
  ...
  const [arama, setArama] = useState("");
  const [analizAcik, setAnalizAcik] = useState(false);
  const [analizMarka, setAnalizMarka] = useState("");
  const [analizHizmet, setAnalizHizmet] = useState("");
  const [analizYasMin, setAnalizYasMin] = useState("");
  const [analizYasMax, setAnalizYasMax] = useState("");
  useEffect(() => {
    if (!hedef) return;
  ...
  const buYil = (/* @__PURE__ */ new Date()).getFullYear();
  const aracYasi = (a) => a.yil ? buYil - (+a.yil) : null;
  const tumMarkalar = [...new Set(liste.map((a) => a.marka).filter(Boolean))].sort();
  const analizFiltreli = liste.filter((a) => {
    if (analizMarka && a.marka !== analizMarka) return false;
    const yas = aracYasi(a);
    if (analizYasMin && (yas === null || yas < +analizYasMin)) return false;
    if (analizYasMax && (yas === null || yas > +analizYasMax)) return false;
    if (analizHizmet && !servisler.some((s) => s.aracId === a.id && s.hizmetTuru === analizHizmet && s.durum !== "iptal")) return false;
    return true;
  });
  const analizServisleri = servisler.filter((s) => s.durum !== "iptal" && analizFiltreli.some((a) => a.id === s.aracId));
  const analizToplamCiro = analizServisleri.reduce((t, s) => t + (+s.tutar || 0), 0);
  const analizYaslar = analizFiltreli.map(aracYasi).filter((y) => y !== null);
  const analizOrtYas = analizYaslar.length > 0 ? Math.round(analizYaslar.reduce((t, y) => t + y, 0) / analizYaslar.length * 10) / 10 : 0;
  const markaDagilimi = Object.entries(analizFiltreli.reduce((acc, a) => {
    const m = a.marka || "Belirtilmemiş";
    acc[m] = (acc[m] || 0) + 1;
    return acc;
  }, {})).map(([name, value]) => ({ name, value })).sort((a, b) => b.value - a.value).slice(0, 8);
  const hizmetDagilimiAraclar = Object.entries(analizServisleri.reduce((acc, s) => {
    const h = HIZMET_TIP_LABEL[s.hizmetTuru] || s.hizmetTuru || "Diğer";
    acc[h] = (acc[h] || 0) + 1;
    return acc;
  }, {})).map(([name, value]) => ({ name, value }));
  const yasDagilimi = (() => {
    const gruplar = { "0-2": 0, "3-5": 0, "6-10": 0, "11-15": 0, "16+": 0, "Bilinmiyor": 0 };
    analizFiltreli.forEach((a) => {
      const y = aracYasi(a);
      if (y === null) gruplar["Bilinmiyor"]++;
      else if (y <= 2) gruplar["0-2"]++;
      else if (y <= 5) gruplar["3-5"]++;
      else if (y <= 10) gruplar["6-10"]++;
      else if (y <= 15) gruplar["11-15"]++;
      else gruplar["16+"]++;
    });
    return Object.entries(gruplar).filter(([, v]) => v > 0).map(([ad, adet]) => ({ ad, adet }));
  })();
  const ANALIZ_RENKLER = [C.accent, C.blue, C.green, C.yellow, C.red, C.steel, C.purple, C.muted];
  return /* @__PURE__ */ React.createElement("div", { className: "fp-fade" }, /* @__PURE__ */ React.createElement("div", { style: { display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16, flexWrap: "wrap", gap: 10 } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 20, fontWeight: 800, color: C.white } }, "🚗 Araç Kayıtları"), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 8, flexWrap: "wrap" } }, React.createElement("button", { style: S.btnO, onClick: () => setAnalizAcik((v) => !v) }, "📈 Detay"), /* @__PURE__ */ /* @__PURE__ */ React.createElement("button", { style: S.btn(), onClick: () => {
    setForm({});
    setModalAcik(true);
  ...
  analizAcik && React.createElement(
    ...
    React.createElement("div", { style: S.secTitle }, "📈 Detaylı Analiz"),
    React.createElement("div", { style: { display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 16 } },
      React.createElement("select", { style: { ...S.sel, width: "auto" }, value: analizMarka, onChange: (e) => setAnalizMarka(e.target.value) },
        React.createElement("option", { value: "" }, "T\xFCm Markalar"),
        tumMarkalar.map((m) => React.createElement("option", { key: m, value: m }, m))
      ),
      React.createElement("select", { style: { ...S.sel, width: "auto" }, value: analizHizmet, onChange: (e) => setAnalizHizmet(e.target.value) },
        React.createElement("option", { value: "" }, "T\xFCm Hizmet T\xFCrleri"),
        Object.entries(HIZMET_TIP_LABEL).map(([k, l]) => React.createElement("option", { key: k, value: k }, l))
      ),
      React.createElement("input", { type: "number", style: { ...S.inp, width: 100 }, placeholder: "Min yaş", value: analizYasMin, onChange: (e) => setAnalizYasMin(e.target.value) }),
      React.createElement("input", { type: "number", style: { ...S.inp, width: 100 }, placeholder: "Maks yaş", value: analizYasMax, onChange: (e) => setAnalizYasMax(e.target.value) })
    ),
    React.createElement(Grid4, null,
      React.createElement(StatCard, { color: C.accent, icon: "🚗", value: analizFiltreli.length, label: "Filtrelenen Araç" }),
      React.createElement(StatCard, { color: C.green, icon: "🔧", value: analizServisleri.length, label: "Servis Kaydı" }),
      React.createElement(StatCard, { color: C.blue, icon: "\u{1F4B0}", value: fmtTL(analizToplamCiro), label: "Toplam Ciro" }),
      React.createElement(StatCard, { color: C.yellow, icon: "\u{1F4C5}", value: analizOrtYas || "—", label: "Ortalama Yaş" })
    ),
    React.createElement(Grid2, null,
      React.createElement("div", { style: { ...S.card, marginTop: 14 } },
        React.createElement("div", { style: S.secTitle }, "\u{1F3F7}️ Markaya Göre Dağılım"),
        RC.PieChart && markaDagilimi.length > 0 ? React.createElement(ResponsiveContainer, { width: "100%", height: 220 }, React.createElement(PieChart, null, React.createElement(Pie, { data: markaDagilimi, dataKey: "value", nameKey: "name", cx: "50%", cy: "50%", innerRadius: 45, outerRadius: 78, paddingAngle: 3, label: ({ name, value }) => `${name}: ${value}` }, markaDagilimi.map((e, i) => React.createElement(Cell, { key: i, fill: ANALIZ_RENKLER[i % ANALIZ_RENKLER.length] }))), React.createElement(Tooltip, { contentStyle: { background: C.card, border: `1px solid ${C.border}`, borderRadius: 8 } }))) : React.createElement("div", { style: { color: C.muted, fontSize: 13 } }, "Veri yok.")
      ),
      React.createElement("div", { style: { ...S.card, marginTop: 14 } },
        React.createElement("div", { style: S.secTitle }, "\u{1F527} Hizmet T\xFCr\xFCne Göre Dağılım"),
        RC.PieChart && hizmetDagilimiAraclar.length > 0 ? React.createElement(ResponsiveContainer, { width: "100%", height: 220 }, React.createElement(PieChart, null, React.createElement(Pie, { data: hizmetDagilimiAraclar, dataKey: "value", nameKey: "name", cx: "50%", cy: "50%", innerRadius: 45, outerRadius: 78, paddingAngle: 3, label: ({ name, value }) => `${name}: ${value}` }, hizmetDagilimiAraclar.map((e, i) => React.createElement(Cell, { key: i, fill: ANALIZ_RENKLER[i % ANALIZ_RENKLER.length] }))), React.createElement(Tooltip, { contentStyle: { background: C.card, border: `1px solid ${C.border}`, borderRadius: 8 } }))) : React.createElement("div", { style: { color: C.muted, fontSize: 13 } }, "Veri yok.")
      )
    ),
    React.createElement("div", { style: { ...S.card, marginTop: 14 } },
      React.createElement("div", { style: S.secTitle }, "\u{1F4C5} Yaş Dağılımı"),
      RC.BarChart && yasDagilimi.length > 0 ? React.createElement(ResponsiveContainer, { width: "100%", height: 220 }, React.createElement(BarChart, { data: yasDagilimi },
        React.createElement(CartesianGrid, { strokeDasharray: "3 3", stroke: C.border }),
        React.createElement(XAxis, { dataKey: "ad", tick: { fill: C.muted, fontSize: 10 } }),
        React.createElement(YAxis, { tick: { fill: C.muted, fontSize: 10 }, allowDecimals: false }),
        React.createElement(Tooltip, { contentStyle: { background: C.card, border: `1px solid ${C.border}`, borderRadius: 8 } }),
        React.createElement(Bar, { dataKey: "adet", fill: C.blue, radius: [4, 4, 0, 0] })
      )) : React.createElement("div", { style: { color: C.muted, fontSize: 13 } }, "Veri yok.")
    )
  ...
  dosyaAdi: "araclar",
  iceAktar: { alanlar: [{ baslik: "Plaka", key: "plaka", zorunlu: true }, { baslik: "Marka", key: "marka" }, { baslik: "Model", key: "model" }, { baslik: "Yıl", key: "yil" }], onIceAktar: aracIceAktar },
  rowKey: (a) => a.id,
  bosMesaj: "Kayıt bulunamadı.",
  rows: filtreli,
  columns: [
...
function App() {
  ...
  return /* @__PURE__ */ React.createElement(
    ...
    if (uzakZaman) localStorage.setItem("fp_son_yerel_degisim", String(uzakZaman));
    window.location.reload();
  } }, "\u015Eimdi Yenile")), /* @__PURE__ */ React.createElement(AktifBilesen, { hedef, hedefTemizle: () => setHedef(null), sayfayaGit, hedefeGit })), /* @__PURE__ */ React.createElement(AsAsistani, { sayfayaGit }));
}
